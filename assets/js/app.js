...
// ---------------------------------------------------------------------------
// 首页的入场动画没有了，所以这里也没有 JS（2026-09-12 撤销，D44 第四节）
//
// 这一格最后一版是 app.css 里那组关键帧加一根 view-timeline，外加这里一小段
// **只量一次**的 JS —— 它算每块在空中的起点，因为 CSS 读不到自己的版面位置。
// 整套删了：用户要的是那两张截图一模一样地连在一起，而入场动画做的正好相反 ——
// 它让下半页在读者滚到之前处在别的位置上。
//
// ⚠️ 留这段话而不是留空：这个效果被推翻过五次，其中有几次是从「先用 JS 试一下」
//    开始的。真要再来一次之前，先去 D44 第四节看那五版各自是怎么被否掉的 ——
//    最后一次否掉的是「有这个动画」本身，不是某一版的参数。
//    守卫：core.tests.TheEntranceStaysGoneGuardTests。

// ---------------------------------------------------------------------------
// `/` 永远从 hero 开始，没有「记住你上次停在哪」这回事（2026-09-12 撤销，D44 第八节）
//
// 这里曾经有一段：把「这台机器上的人见过那一下没有」记在 localStorage 里，
// 第二次起打开 `/` 就自动滚到仪表盘那一屏，hero 留在上面。
//
// 撤掉它的是顶栏正中那个字样。它指向 `/`，而它在任何网站上的意思都是
// **「回到首页」** —— 点下去却落在仪表盘上：
//
//     点吸顶的 "river of life foundation" 竟然跳到 home page 了，
//     应该像之前一样去 hero 首页的。
//
// 🔴 教训不在那一个控件上：**这一页只有一个地址，于是「打开 `/`」这件事从此有
//    好几种来意** —— 刚登录完、从菜单回来、点字样回首页、自己敲地址。那段脚本
//    对它们一视同仁地改写落点，所以它必然会在其中某一种上读成 bug。要么每一种
//    来意各自决定落在哪（那是一张没人要的表），要么一种都不改写。
//
// ⚠️ 代价如实记：天天用的人现在每次打开 `/` 都要往下滑一次才看见仪表盘。
//    这是知道的、选择付的 —— 换来的是「打开首页看见首页」没有例外。
