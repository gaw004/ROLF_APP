...
图例：**✅ 做完并验过** · **🟡 代码做完，差一次真实验证** · **⬜ 未开工**。

⚠️ 这张表的判据是[本阶段的完成定义](phase-c.md#一这一阶段要达成什么)，
不是「代码写完了」。一步只要还剩一次拿真账号 / 真收件人 / 真库的验证没走，
它就是 🟡 —— 而这个项目的记录里，「服务层写好了、没有页面」那五处缺口
正是把 🟡 当 ✅ 记下来的结果。

| 步 | | 还剩什么 |
|---|---|---|
| [C0.1](#c01-重建本机库) 重建本机库 | ✅ | |
| [C0.2](#c02--补齐-14-条的功能缺口) 补齐 14 条的缺口 | ✅ | 五处全补齐，含 C0.2.6 的返工 |
| [C0.3](#c03-三角色浏览器验收) 三角色浏览器验收 | ⬜ | **Phase B 结案就差这一遍**。等的是人，不是代码 |
| [C0.4](#c04-修文档漂移phase-b-标-) 修文档漂移 | 🟡 | 三项里两项做了，第 3 项（Phase B 标 ✅）卡在 C0.3 |
| [C0.5](#c05--上线前的三条死链) 三条死链 | ✅ | |
| [C1](04-roadmap.md#c1--构建链) 构建链 | ✅ | |
| [C2](04-roadmap.md#c2--设计系统与-20-个模板) 设计系统与 20 个模板 | ✅ | |
| [C3.0](#c30-域名--发信服务--最先做因为它靠别人) 域名 + 发信服务 | ✅ | 域名已买；Brevo 域名认证做成了 —— `dkim=pass header.d=rolf-v.org` + `dmarc=pass` 是收件方给的证据，不是面板上的绿勾 |
| [C3.1](#c31-首页-) 首页 `/` | ✅ | |
| [C3.2](#c32-密码重置) 密码重置 | ✅ | 线上走通了：注册 → 收信 → 改密 → 登录 |
| [C3.3](#c33-真实发信) 真实发信 | ✅ | 线上验过。⚠️ 头一封发给高校网关（Proofpoint）时被隔离了 —— 认证三项全过，是新域名的信誉问题，记进了[已知缺口](phase-c.md#五已知缺口与处置) |
| [C3.4](#c34-生产加固) 生产加固 | ✅ | 线上实测 `check --deploy` 零警告（2 条按值静默，C5 会让它们自己回来） |
| [C3.5](#c35-部署到-render) 部署到 Render | 🟡 | 服务在跑、域名挂上了、环境变量齐了。差**两个账号和基础数据** —— 跑一次 `manage.py check_deployment` 就有答案 |
| [C3.6](#c36-备份--恢复演练) 备份 + 恢复演练 | 🟡 | 🔴 **交付硬前置。** 脚本和本机演练做完了，**真 R2 + 真生产库那一遍没走** |
| [C3.7](#c37-权限复核) 权限复核 | ⬜ | 🔴 **交付硬前置。** 四件事全要拿真账号做 |
| [C3.8](#c38-错误可见性) 错误可见性 | 🟡 | 代码和 DSN 都就位。差「线上故意打一个 500，Sentry 里看得到」 |
| [C3.9](#c39-登录限流) 登录限流 | 🟡 | 注册和密码重置两处限流都做了；`django-axes` 管的登录爆破没做 |
| [C3.10](#c310-一页隐私说明) 一页隐私说明 | ⬜ | 🔴 **交付硬前置。** 内容要基金会确认留存期限和删除流程 |
...
**实测结果（2026-08-17，线上验过）**：

| 项 | 结果 |
|---|---|
| 供应商 | Brevo SMTP relay，587 / STARTTLS |
| 域名认证 | ✅ 收件方的邮件头是硬证据：`dkim=pass header.d=rolf-v.org` · `spf=pass` · `dmarc=pass header.from=rolf-v.org` |
| 发件地址 | `noreply@rolf-v.org` —— 自己的域名，不是供应商的共享子域 |
| 端到端 | 线上走通「注册 → 收密码重置信 → 改密 → 登录」 |

⚠️ **第一封信被高校网关（Proofpoint）扔进了隔离区**，而认证三项全过 ——
所以那不是配置问题，是**一个全新发信域名的信誉问题**：`rolf-v.org` 那天为止
一共发过 1 封信。处置记在
[`phase-c.md` 的已知缺口](phase-c.md#五已知缺口与处置)：不为它做任何特殊处理
（换域名、加免责声明这类动作通常更可疑），但试点期间有人说「没收到」时，
第一句要问的是「你的邮箱是不是学校或公司的」。

⚠️ 顺带记一条**关于测试信本身**的：`check_deployment --send-to` 发的那封
极短、无结构、无退订、主题写着 "delivery test"，**它比真实邮件更像垃圾邮件**。
别拿它的命运去推断密码重置信的命运 —— 反过来才对。
