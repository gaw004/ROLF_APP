...
1. **[D32 的唯一不变量](decisions/D32-worker-axes-schedule-and-assignment.md)**：
   在编只有一套结构（`Position` + `Assignment`），不许有第二套。
   本轮任何一步如果开始出现"给这类人单独建一张表"的念头，先回去读那一节。
   ⚠️ 它说的是路径不是条数 —— 一个人可以同时持有多条 `Assignment`，
   判据一律写成"存在一条满足条件的在职任职"（这一句 2026-08-21 补进 D32，
   起因是原来那句标题被读成了"一个人只能有一行任职"）；
...
| **D2a** 班表结构与自助 | `WorkPattern` + `Shift` + **生成器（⚠️ 调 `events/recurrence.py`，别再写一个 —— 见下）** + cron + **批量建模板** + **My Schedule + `.ics`** | 员工第一次能看到自己的排班。⚠️ 到这里为止**只读**，不碰出勤和工时口径（`staff_hours()` 的 `scheduled` 一档除外，见 D2a.6） |
| ⏸ | 试点反馈 | 见下 |
...
> ### ⚠️ D2a 的生成器已经有人写了（2026-09-08 补）
>
> [`06-roadmap.md`](06-roadmap.md) L5.5 建 `events/recurrence.py` —— 一个纯函数
> 展开器，`dateutil.rrule` 打底，**不引入新依赖**。它的 docstring 里写着三个
> 调用方，第三个就是本阶段的 `WorkPattern` 生成 `Shift`。做 D2a 时调它。
>
> 分歧只在「生成到哪一天为止」：活动的系列必须带结束条件、一次生成完，班次是
> 滚动窗口 + cron。那是[D33](decisions/D33-work-schedule.md) 末尾那一节讲的
> 判据（这件事有没有自然的终点），和「怎么展开一条规则」无关 ——
> 后者只该有一份实现。
...
> ### ⚠️ 2026-09-08：和参与者那一轮的关系（两步正交，顺序不限）
>
> [`06-roadmap.md`](06-roadmap.md) 的批一让 `attending` 的行**根本没有工时**
> （`served_as=not_applicable` + 一条数据库约束），本步是把**剩下的**工时拆成
> 志愿 / 工作。两件事作用在不同的行上，叠加不冲突，先做哪个都行。
>
> ⚠️ 但本步落地时，报表上那几个工时口径已经**不是**当初写这一节时的样子了：
> 分母收窄成「来帮忙的人」、多了一个 `people_served`、标题改成
> Hours per helper。改之前先读 [D27](decisions/D27-ministry-report.md) 那个
> 2026-08-21 的框，别照这一节的旧描述去动。
