...
#### 验收（浏览器，✅ 2026-09-14 走完）

- [x] `/events/` 上没有那门课；顶栏 `Events · Programs` 点得过去，字不横着挪
- [x] `/programs/` 上只有课，右边日程画的是**每周那一格**，不是 111 天的横条
      —— 实测画出来的是周一晚上一个两小时的方块，带「Session 6」
- [x] 列表行上不再出现 `Aug 9, 4:05 p.m. – Nov 7, 3:05 p.m.`，改成日期区间 + 一句「每周二 …」
- [x] 从 `/programs/` 点进一门课再点返回 → 回到 `/programs/`，那一行还在
- [x] 不带任何标记直接打开详情页 → 返回键同样是 `← Programs`（读的是 `event.shape`）
- [x] 站点菜单里 Events 底下有 Programs
- [x] My Signups：Next up 三格带「meeting 7」、课是卡片、单场是表格、底部两个数并排没有合计
- [x] 进度方块四态在真数据上都出现了：5 出席 · **1 未点名** · 6 未来，今天那一格带环
- [x] 筛选 `All · Events · Programs` 切得动，且**筛完那一排还在**（回得去）
- [x] Past Signups 排版一样、没有 Next up、工时和签到两列只在那一页
- [x] 窄屏 500px：Next up 叠成三行，整页无横向溢出

#### 🔴 走查抓到两条，两条都是「HTTP 那一遍十三条全过」之后才现形的

| | 症状 | 为什么 HTTP 验不出 |
|---|---|---|
| 1 | Programs 那一页在浏览器标签、书签、历史里**都叫「Events」** | `{% templatetag openblock %} block title {% templatetag closeblock %}` 写死了。页面**正文一个字都不错** —— 它错在 `<title>`，而那个东西只有把页面画出来才看得见 |
| 2 | Past Signups 上那一段写着**「Events coming up」**，底下挂的是上个月就结束了的活动 | HTTP 那一遍问的是「两段在不在」，答案是「在」。错的是**段落叫什么** —— 一句被它自己旁边那个日期当场证伪的话 |

两条各配了一条测试（`test_each_list_names_itself_in_the_browser_tab`、
`test_each_section_heading_follows_the_page_it_is_on`），都在旧代码上验过会红。

⚠️ 还有一条**差点报错的**：窄屏下按 Schedule 那颗键日程不开，第一反应是 bug。
量了一下 `window.innerWidth` 才发现视口是 500px —— 而日程面板在那一档按设计就是
藏着的，`/events/` 上同一颗键行为一模一样。**截图看着像宽屏，而断点认的是
`innerWidth`**：走查时凡是和断点有关的判断，都要去量那个数，不能信眼睛。
...
### L5.8h 续排：生成的下界 + 通栏那一行（2026-09-17 交付）

用户问的是一句话：「recurring event 一年期限快要到了，会提前多久通知 admin？
不能过期以后再让 admin 续上吧」。
...
`RUNNING_LOW`（六周）和 `is_running_low()` 从 L5.8e 起就在，而它的**全部形态**
是系列页上那行会变黄的字。它要 admin **主动打开那一条系列的页面**才看得见
—— 没有邮件、没有通栏、没有定时任务。而滚动生成的整条设计靠的正是他记得回来按。

#### 🔴 真正的代价不在「续不上」，在「续上的那一下」

`generate_occasions()` 的窗口**只有上界没有下界**：`_occasion_moments()` 从
`starts_on` 一路排到一年后，生成器照单全收。于是一条断了两个月的系列，在有人
想起来按的那一下，会把断掉的那八个周二**建成真的活动** —— 发布出去、带着工种、
被牧区报表算成「开了但没人来」的聚会。而且删不掉：`_collectable_occasions()`
按设计不碰已经开始的场次，撤销够不着，只剩手工删八行。

`EventSeries.clean()` 里那段注释早把这个后果写全了，但它只在系列**还一场都没有**
的时候拦（`self.pk is None or not self.occasions.exists()`）—— 一条跑了一年的
系列正好落在它够不着的地方。
...
| | 定案 | 为什么 |
|---|---|---|
| 下界 | **跳过**，并在结果句子里说清 | 拒绝整次按键在这里是死胡同：`starts_on` 被 `_refuse_rewriting_the_rule()` 冻着，改不了，于是这条系列再也续不上 |
| 判据 | `moment > now`，一个**瞬间**不是一天 | 和 `_top_up_roles()`、`_drop_generated_after()` 同一列同一种问法（「它开始了没有」）。那两处各自写过为什么这一块不适用通常那条 `end_time` 规矩；这里是第三个读者，不新发明第三种口径 |
| 提醒的第二个落点 | 仪表盘通栏 `Needs you` | 那是 admin 的必经之路。判据仍是同一个 `is_running_low()`，两处不许有两个口径 |
| 名额 | 仍是 `BAND_ITEMS = 3`，**排在「还缺人」之前** | 缺人的岗位别的 admin 也补得了；重复规则只有这个牧区的 admin 按得动，而且过了期不可逆 |
| 窗口 | 六周**不变** | 两处共用一个数 |

⚠️ `already_gone_sentence()` 是「跳过」这个做法唯一的辩护 —— `clean()` 那段注释
反对过跳过，理由是「『为什么只有三场』到时候在页面上没有地方答得出来」。
那条反对成立，所以答案就是那句话，而且**两个按得动生成的入口都要说**
（系列页那颗键、Django admin 那个批量动作，后者自己拼句子）。

#### 预览也要跟着改，否则这一块自己开始说谎

日期预览和生成器从**同一个** `_occasion_moments()` 拿日期 —— 那正是它当初被
这么写的理由。加了下界之后，预览列出全部历史日期、上面写着「53 occasions，
4 Sep 2024 到 17 Sep 2027」，而按下去一场过去的都不会造：页面说一个数、
按钮做另一件事，也就是本节反复点名的那类失败的第二个入口。

所以切线抽成 `gone_and_to_come()`，预览和生成器共用它，预览多说一句
「其中 N 天已经过去了 —— Generate 只建还没到的那些」。

⚠️ 措辞**不是**「按下去会造 N 场」：那在一条健康的老系列上是假的（那些场次
早就在库里了，按下去一场都不会多）。这一块答的是「规则落在哪几天」，
「按下去会发生什么」由按完那句消息答。

#### 查询预算：12 → 15、20 → 21，而这是买东西花掉的

通栏那一块不能逐条问 `is_running_low()` —— 那是每条系列两次查询，一个管三个
牧区的 admin 打开首页就能吃光预算。`series_running_low()` 收 queryset、
只筛（D27），**两次查询**并且不随系列条数涨：一次带聚合的筛选把「最后一场还很远」
的挡在 SQL 里，一次把剩下那几条的场次时刻取回来，然后把 `booked_to` /
`standing` 递给 `is_running_low()` —— 它于是一次查询都不花。

实测：没有快排完的系列时 13，有一条时 **14**（第二次查询只在真有候选时才跑）。
上限按**量出来的最贵那一形态**定，留一格。

⚠️ 顺带修掉的一件事：`deferred.md` 那条「三条在午夜前后会自己变红的测试」记的
夹具问题。那三条如今**不复现**（2026-09-17 在它描述的那个时间窗里全量跑过，
2633 条全绿），而它们的夹具确实靠 `NOW`（导入时的 UTC 瞬间）和 `local_today()`
两个基准去跨越今天。下界落地之后它们必须改成 `generate_as_of()`
（「三周前按的那一下」），于是那个两基准的依赖一起没了 —— 不是按那条账修的，
是被下界逼出来的。
...
| | 症状 | 真因 |
|---|---|---|
| 🔴 | 发布一门**课**的页面上印出「Generate only builds the ones still to come」—— 而那是**两重假话** | 日期预览三页共用，第一版让 `_dates_context()` 无条件算那个数。课那一支**没有下界**：`ProgramForm.clean()` 不拒绝落在过去的日期（系列那边靠 `EventSeries.clean()`，课没有那张表），`publish_program()` 给每一个时刻都建一讲；而且那一页上根本没有 Generate 这颗键。⚠️ 我写的模板注释当时断言「课那一支同理（clean() 会拒）」—— **一句断言某个保护存在、而它并不存在的注释**，正是本仓库反复判刑的那一类 |
| 🔴 | 一条办了两年的系列，每渲染一次首页就把它**全部的历史场次**搬进内存 | `series_running_low()` 的筛选没有下界（也不能有 —— 断了的系列 `booked_to` 正在过去，那恰恰是要找的），于是每条早就办完的系列都落进候选，第二次查询再把它们全部场次取回来。而 `has_more_to_build()` 只会问「还没到」的那些，过去的一行都不会被用到。⚠️ **查询预算看不见它**：条数不变，变的是每条搬多少行 |
| 🔴 | 「Nothing new to make — all 39 occasions already exist」紧接着「8 dates … were not made」 | 自相矛盾，而且 39 根本不是这条规则要的场数（它要 61）。为这种系列写的那一支只在**一场都没有**时才够得着 |
| ⚠️ | `EventSeries.clean()` 那句拒绝描述的后果**不再发生** | 它写着「生成会把这些已经过去的晚上建成真的活动，而且撤销不回来」—— 有了下界之后不会了。拒绝本身留着，但**理由换了一个**：它挡的不再是数据损坏，是一个说不通的排法（新系列的第一场在三个月前，那几场永远不会存在） |

⚠️ 第二条的修法有一半是**问不出来的**，如实写着：「即日停止」过的系列可以在 SQL
里挡掉（`ended_on` 本来就会截断展开），而 `COUNT` 跑完、没有 `ended_on` 的那一半
SQL 问不出来，仍然要展开一次才知道。

⚠️ 第一条修完之后那一格是 `skips_the_past`，**默认关**，由两个系列调用方开口要
—— 和隔壁 `noun` 的默认方向相反，而这是有意的：`noun` 漏传只是名词不贴切，
这一格漏传是印出一句假话。

#### 浏览器走查

- [x] 一条断了两个月的系列 → 通栏第一格「Tuesday prayer meeting / Booked only to 21 Jul 2026」，**排在两个缺人的岗位之前**，整块仍是 3 items 一行
- [x] 点进去 → 「Booked through 21 Jul 2026. That is not far off」+「61 of them have already gone — Generate only builds the ones still to come.」
- [x] 按生成 → 「52 occasions generated … Booked through 14 Sep 2027. 8 dates between 28 Jul 2026 and 15 Sep 2026 had already gone and were not made」
- [x] 回仪表盘 → 那一行消失，而**被它挤掉的第三个缺人岗位回来了** —— 名额那条规矩两个方向都成立

⚠️ 走查当场看见的一件事，**这一轮没改**：上面那句「113 occasions over the next
year, 22 Jul 2025 to 14 Sep 2027」在一条跑了两年的系列上读着别扭 —— 它其实是
「这条规则在窗口里落的全部日期」，而「over the next year」是照着一条**新**系列
写的话。新加的那一句把它救回来了（说清其中 61 天已经过去），但那句话本身仍然
不准。记进 `deferred.md`。
...
| 界面 | `<details>`，无 JS 可用（D24）。三个去处**不画成等价的**，各带一个标志（下载 / Google / Outlook），⚠️ 后两个是简化近似版不是官方品牌资产，要换的话只动那一个 partial。⚠️ 那一排上一度还有第四条「Next meeting only」，当天就删了 —— 讲次表开放之后它成了「任意一讲」的一个硬编码特例 |
| 挑哪一讲 | 讲次表上每一讲各一个入口，而那张表 2026-09-14 从「只给管理员」**拆列开放**：第几讲 / 什么时候 / 加进日历人人看得见，来了几个人 / 签到屏仍然只给管理员 |
...
### 验收（浏览器，2026-09-14 走了大半）

- [x] 一门课 →「All meetings (.ics)」→ 文件里**十二条** VEVENT，不是一条三个月的
- [x] 同一页 →「Next meeting only」→ 只有一条
- [x] 详情页那颗键展开是**四条链接 + 一行小字**，小字说得出「文件会更新已有那条、
      两条深链各加一个日期且每次多一条」
- [x] 列表和卡片上是**一颗小图标**（纯 SVG 无文字，用户当天的要求），
      `aria-label` 各带活动名 —— 十几行上十几个同名按钮，读屏的人分不出哪个是哪一行
- [x] 响应头三样都对：`text/calendar`、`attachment; filename="esl-class.ics"`（带活动名）、
      `Cache-Control: private, no-store`
- [x] 文件里那句「This entry will not change if the event does」在
- [x] 看不见这场活动的账号敲那个地址 → 404（测试覆盖）
- [x] **学员也挑得了任意一讲**：实测入口数从 2 变成 14（12 讲各一个 + 整期 + 下一讲），
      而签到屏和「来了几个人」那一列他仍然一个都看不到

#### ⚠️ 一条理由被新功能推翻，而它当初是对的

讲次表原来只给管理员，那一行的理由写着「对报名者『这门课什么时候上』已经答在
When 那一行上了」。**那句话在加进日历之前成立** —— 之后报名者要问的变成了
「**第七讲**是哪天，我想单独把它放进日历」，而页顶那行 When 答不了：它说的是
整学期的节奏，不是某一次。

🔴 记下来是因为它不是「当初写错了」：**一条理由会随着页面上多出来的东西失效，
而失效的时候它读起来还是很有道理**。拆的是列不是表 —— 两张表会在下一次加一列时
分叉，而它们说的是同一批行。
- [ ] ⚠️ **`.ics` 在 Apple 日历里真的打开过一次** —— 这一条**没走**，
      浏览器走查到不了那一步（要把文件交给系统日历）。它是这一整套里唯一
      「格式对不对」的**终局判据**，留给试点那一天
- [ ] ⚠️ **再下一次不多出第二条** —— 同上，要一个真日历才验得了。
      UID 稳定这件事有单元测试钉着（`ICalendarTests`），但「客户端认不认」是另一回事
- [ ] 改掉课名 → 重新下载 → 那十二条**跟着改名** —— 同上
- [ ] Google / Outlook 两条深链真的打开对的草稿 —— 要连外网，本轮没走

### 订阅：一条会自己跟着变的日历地址（2026-09-14，用户提的）

🔴 **它补的正是上面那一整套的洞。** 下载下来的 `.ics` 是活动时间地点的第二份
拷贝，住在别人手机里，基金会一改期它就在那个人唯一会看的地方静默地假 ——
那段代价小字说的就是这件事。订阅不会：每次刷新重新取，退掉的自己消失，
加的讲次自己出现。

| | |
| --- | --- |
| 粒度 | **每人一条私密地址，内容是「我报的全部」**（用户定的，另外两个选项是每门课一条、以及公开的按活动一条） |
| 模型 | `events.CalendarFeed` —— 一对一挂在 `Contact` 上，256 位随机令牌，可撤销 |
| 路由 | `calendar/<slug:token>.ics`，**没有 `@login_required`** |
| 界面 | My Signups 底下一块 `<details>`；默认只有一颗「Create」的键 |
| 内容 | `services.my_calendar_occasions()` —— 走 `Participation.objects.mine()`，和这一页同一个谓词 |

🔴 **五条只有实施才撞得上的，逐条记下来：**

1. **抓订阅源的是 Google / Apple 的服务器，它不带 cookie。** 照现成那条
   `@login_required` 的路由订下去，对方拿到的是登录页的 302 —— 订阅「成功」，
   日历里一条都没有，**而且不报错**。这是整个功能的形状由它决定的那一条：
   门不能是登录态，只能是地址本身。
2. **「这份不会自己更新」在订阅源里是一句假话**，而它假得正是本仓库反复在拆的
   形状：每个字都对、不报错、没有测试会红，而读的人会照它行事（每次回站里
   核对，订阅白订）。所以 `ics.calendar_for()` 多了一个 `updates=`，把那句话换成
   成立的那一句 —— **换，不是省**：模块仍然无条件交代「这份拷贝有多新」。
   ⚠️ `updates=False` 是围栏：它是整个模块里唯一能让那句话跑偏的地方，默认站在
   两条现成下载路由那一边，它们因此一个字都不用改。
3. **`cancel()` 是改状态不是删行**，所以「退掉的自动消失」**不是白来的**。
   一门课有一半是自动的（`cancel()` 会清掉还没发生的讲次的点名行），而**单场
   活动**的 occasion 来自 `Event` 自己，不排除状态的话它会永远留在订阅里。
   两种形状用同一条规矩：`CANCELLED` / `WITHDREW` 显式排除，测试特意用单场活动写。
4. **不在 `Contact` 上加一列，而理由是不要凭空发钥匙。** 加一列等于给库里每一个
   人（包括永远不会订阅的那几千个）签发一条真能用的凭证。一张表 + 一颗「Create」
   的键，才是真的懒发。⚠️ 守着这件事的只有一条测试
   （`test_merely_looking_at_the_page_never_issues_an_address`）——
   改成 `get_or_create` 之后页面照常渲染、别的测试全绿。
5. **不走 `events/tokens.py` 那套 HMAC。** 那一套是 90 秒的签到码：纯函数、
   不落库、**撤不掉**。而这里要的恰恰是撤得掉；签名要能撤就得为每个人存一份盐，
   那时它已经是这张表了，只是绕了一圈。

⚠️ **「换一把钥匙」走确认页**（照 `participation_cancel` 的先例），理由是它的
后果**静默**：订着旧地址的日历不会报错，只是停在原地，人要过一阵子才发现课表
不动了，而那时多半不会把两件事联系起来。

⚠️ 那行小字里两句都不能省：这条地址**就是一把钥匙**（人是会转发的）；
以及**之前下载过 `.ics` 的人会看到同一个晚上出现两次** —— 两份住在两个日历里。
后者不是 bug，是两条路同时存在的必然结果，但人不会这么理解。

#### 验收（2026-09-14）

- [x] **不带任何 cookie** 请求那条地址 → 200，里面是这个人的场次
- [x] 乱令牌 → 404（不是 403 —— 403 等于确认「这个形状的地址是真的」）
- [x] 换一把钥匙之后，旧地址 → 404，新地址 → 200
- [x] 响应头：`text/calendar`、`private, no-store`、`noindex`，且**没有**
      `Content-Disposition`（带上它，一部分客户端会存成文件而不是订上它）
- [x] 文件头上有 `X-WR-CALNAME` 和 `REFRESH-INTERVAL;VALUE=DURATION:PT12H`
- [x] DESCRIPTION 里是「updates itself」，而**下载的那一份仍然是「will not change」**
- [x] 同一场活动里两个角色 → 一组 VEVENT，不是两组
- [x] 撤回发布 → 下一次刷新就从订阅里消失
- [ ] ⚠️ **真的在 Apple 日历 / Google 日历里订上一次** —— 没走，同上面那一套的
      理由：它是「格式对不对」和「客户端认不认」的终局判据，留给试点那一天
- [ ] ⚠️ **改一次课的时间，看订阅方多久跟上** —— 同上，要一个真日历才验得了

#### ⚠️ 同一天第三轮：每一讲自己那颗「⋯」撤掉了

上午定下「讲次表每一行各给一个入口」，下午给每行配了一个 `<details>` 把三个去处
收起来，再给表头加一颗「一次全开」的键。晚上用户判：**收纳撤掉，键留着。**

理由一句话：要点两次才够得着一个本来就该在那儿的链接 —— 先展开这一行、再点图标
—— 而它收起来的是三个 16px 的标志。

🔴 **撤掉的是收纳，不是能力。** 单讲的 `.ics`（`session_calendar`）全须全尾地留着，
它正是「打开之后下载自己想要的那一讲」的那一下。

连坐删掉的三样，以及为什么**必须一起删**：

1. `_meeting_calendar_menu.html` 整个文件 —— 连同它那段「为什么是 `<details>`
   不是 `popover`」的论证。那段话的第①条理由写的是「那颗键的全部意思就是一次
   全打开，而 `popover="auto"` 同时只开一个」；键还在，但它现在开的是一个 class，
   **理由连同前提一起不成立了**。⚠️ 留一段解释不存在之物的注释比没有注释贵。
2. `.meeting-calendar` / `.meeting-calendar-trigger` 两组规则。
   ⚠️ **`.row-menu-dot` 留着** —— 管理行那颗 ⋮ 复用同一个点。跟着一起删掉它的
   表现是全站管理列表每一行的 ⋮ 变成三个不可见的空 span：不报错、不动版面、
   测试全绿，只有人去看那一页才发现按钮没了。
3. `test_one_button_opens_every_meeting_at_once` —— 它钉的是十二个 `<details>`。

🔴 **收起这一下现在由 JavaScript 写，而这不是偷懒，是 D24 逼出来的。**

表头那颗键带 `x-cloak`，没有脚本时**根本不画**。所以把 `meeting-calendars-collapsed`
烤进模板，等于让这三条链接对关掉 JavaScript 的人**永远不存在** —— 而页面一个字
不差、不报错。于是：服务端发出去的是摊开的，紧跟在表后面的一段内联脚本在首帧
之前把它收起来（同 `_period_filter.html` 的做法，只是这里脚本贴在表后面，
`previousElementSibling` 就够得着，不必绕到 `<html>` 上）。

⚠️ **已知代价，写下来免得以后当成 bug 修**：引导脚本跑了、而 `app.js` 那个包整个
没起来（404 / CSP / 报错）时，表是收着的而键不画，那三条链接就够不着了。这个暴露面
不是这里引进的 —— 站里每一个 `x-cloak` 元素在 Alpine 没起来时都是永久
`display: none`。认下它，是因为躲开它就得把收起推迟到包加载之后，而那是每一页都
看得见的一次闪动，换一个从没发生过的分支。

#### ⚠️ 收起用 `visibility` 不用 `display`：一点开关，十二行不许跳

用户看着页面提的。`display: none` 时这一格在收起态是空的，而那三个图标
（16px 加上下各 4px 内边距 = 24px）比同一行的文字（约 21px）**高** ——
于是按一下 Show calendars，每行长高 4px，整张表往下跳一下。

`visibility: hidden` 让盒子照占位置：收起和摊开的几何完全一样，纵横都不动
（实测十二行的 y 坐标两态一致）。顺带拿到的两件事都是要的 —— 藏起来的链接
不进读屏的朗读，也按不到 Tab。

⚠️ **不要改成「把内边距缩到比文字矮」来省这一下**：那样图标的可点区就小于
24px，而这一列在触屏上本来就难点。
守卫：`CalendarDownloadTests.test_collapsing_hides_them_without_moving_the_rows`，
断言落在样式表上 —— 这件事只有 CSS 说了算，服务端 HTML 里一个字都看不出来。

#### 🔴 走查抓到的：同一个开关**写错了两次**，而两次服务端测试都是绿的

收起那一下要由 JavaScript 做（上面那条），落点是 wrapper 上一个 class。
写法试了三种，前两种都不报错、都通过了全部测试、**都在浏览器里是坏的**：

| 写法 | 表现 |
| --- | --- |
| `x-bind:class="open ? '' : '…collapsed'"` | 按一下键，字从 Show 变成 Hide，**十二行一个图标都没出来** |
| 上面那个 + `x-init` 先摘掉它 | **进来时就是摊开的**，而那颗键写着 Show |
| `x-init="$watch('open', …classList.toggle…)"` | 对 |

原因是 Alpine 的两条性质，各自都不显眼，凑在一起就成了这个：

① **`x-bind:class` 只移除它自己加过的 class。** 这一个是引导脚本直接写进
   `classList` 的，Alpine 初始化时见它已在、不记账，于是后面也删不掉它。
② **指令顺序里 `bind` 排在 `init` 之前。** 所以「init 里先摘掉、让 bind 重新
   加一遍并记账」这个修法是反的：真实顺序是 bind 先看（不记账）、init 再摘，
   净效果是那个 class 被摘掉之后再没有人加回来。

`$watch` 绕开整本账：`open` 是唯一的真相，class 由它派生，初始那一份由引导脚本
给（首帧之前，不闪），之后每次变化 toggle 一次。
⚠️ 不用 `x-effect` —— 它会把读到的所有东西都变成依赖，这个仓库为同一条理由
已经写过两次（`event_list.html` 和 `app.js`）。

🔴 **这一条最值得记的不是 Alpine 的脾气，是它暴露了测试覆盖的边界：**
三种写法**发出去的 HTML 在断言层面区分不开** —— 差别只在浏览器里跑起来之后。
服务端测试能守住的是「收起的 class 没有烤进模板」（那一条真的守住了），
守不住「按下去到底动不动」。所以那条「提交前跑起来看一眼」不是走过场，
本轮它就是唯一发现这两个 bug 的东西。

#### 🔴 顺带：这一轮写出了一条**假绿的守卫**，反向验的时候才抓到

`test_the_response_itself_never_ships_them_collapsed` 第一版长这样：

```python
wrap = re.search(r"<div[^>]*\btable-wrap\b[^>]*>", html)
self.assertNotIn(COLLAPSED_CLASS, wrap.group(0))
```

两个毛病叠在一起，而它们**互相遮掩**：

① 这一页有**两个** `.table-wrap`（上面那张角色表、下面这张讲次表），
   `re.search` 给的是第一个 —— 断言落在了一个根本不相干的元素上；
② 就算匹对了它也会**永远红**：`x-bind:class` 属性里本来就写着这个类名，
   那是 Alpine 接手之后的那一份，正当存在。

一条永远绿、和一条永远红，合起来表现为「看上去在守、其实什么都没守」。
把 class 烤进模板跑了一遍，它**没有变红** —— 这就是那条规矩的全部用处：
**新写的守卫要在它该红的那棵树上跑一遍。** 改法是按 Sessions 小标题定位，
断言只落在静态的 `class="…"` 里。

## 钉住的菜单：一条只有图标的窄栏（2026-09-14，用户提的）

| | |
| --- | --- |
| 钉的是什么 | **左侧滑出的那块面板**，不是顶栏（用户在两个选项里挑的） |
| 和正文的关系 | **浮在正文上**，正文一个像素不动 |
| 钉住时的样子 | 一条**竖直居中、长度刚好装下图标**的浮起 bar（4.5rem 宽），每个图标下面一行小字 |
| 记在哪 | `localStorage`，key `menu:pinned`，首帧由 `<head>` 里的引导脚本写 |

#### 🔴 2026-09-15 改：不再顶天立地，是一条竖直居中的浮起 bar

用户改的主意。原来它是整条 `inset-y-0` 的侧栏，从屏幕顶铺到底；现在是**竖直居中、
长度刚好装下那些图标**的一条（实测 565px 高，1440×900 上上下各留 56px），
左边留 8px、圆角 16px、带投影 —— 让它读起来是一块**浮着的东西**，而不是一条
没画完的侧栏。

⚠️ **居中用 `translate` 这个独立属性，不是 `transform`。** 上面那条
`.home-menu.is-open` 会写 `transform: translateX(0)`，用 transform 居中的话两者
互相覆盖 —— 表现是钉着的时候一旦 `menu` 变真，这条 bar 跳回竖直原点。
`translate` 和 `transform` 各自独立合成。（实测验过：加上 `.is-open` 之后 top 不变。）

⚠️ `bottom: auto` 不能省：基础那一档写着 `bottom: 0`，留着它高度仍被拉满，
`top: 50%` 只会把它推到下半屏。
⚠️ `max-height: calc(100vh - 1.5rem)` 是矮视口的兜底 —— 417px 高的窗口上实测
封顶到 393px、上下各留 12px、内部自己滚。

🔴 **「浮在正文上」这条选择，决定了它只能是一个宽屏的东西 —— 这不是偷懒，是几何。**

浮着就不推开正文，那么在**没有空白边距的宽度上，这条 bar 必然压住内容**，没有
第三种可能。**这一版的数（旧的那组随「顶天立地」一起作废了）：**

- bar 的右缘 = `left` + `--menu-rail-w` = 8 + 72 = **80px**
- 最宽的那一档版式（`max-w-6xl` = 1152 + `px-4`）在 1280px 上正文从
  `(1280 − 1152) / 2 + 16 = 80px` 开始 → **正好不压**

⚠️ 左边那 8px 因此是**量出来的上限，不是随手挑的间距**：先写的 12px，实测
（报表页，1280px）右缘 84px、压进正文 4px。改 `--menu-rail-w` 要连它一起重算。

⚠️ 顶栏那颗 Menu 键仍然收掉，但**原来两条理由里的第一条已经作废** ——
它说的是「1280–1376 之间窄栏正好盖在它身上」，而这条 bar 竖直居中、根本不碰顶栏。
剩下的第二条单独就够：**这条 bar 就是那个菜单**，每一项的名字都写在图标底下。

⚠️ 1280px 以下，`<html>` 上那个 class 还在但整段 CSS 不生效 —— 菜单退回滑出的
样子，一切照旧，窗口拉回来它又回来，**不需要任何 JS 参与**。钉不了的宽度上
那颗 Pin 键整个不画：一颗按下去什么都不会发生的键比没有更糟。
（实测过 1440 / 1024 / 390 三档。1024 那一档在改断点之前是坏的：顶栏左上角只剩
半个「nu」，活动卡片的缩略图被切掉一条。）

### 🔴 同一个坑栽了四次：层的优先级高于选择器具体度

Tailwind 的工具类在 `@layer utilities`，面板这些组件类在 `@layer components`。
于是**模板 class 串上的任何一个工具类，都压得过 app.css 里那条随状态变的规则**，
而三次的表现各不相同、一次都不报错：

| 元素 | 留在模板上的 | 表现 |
| --- | --- | --- |
| 面板 | `w-full max-w-sm px-8 py-6` | 钉住之后面板照旧 24rem 宽，而里面的图标已经按窄栏排好了 |
| Close | `flex` | 窄栏顶上 Close 和 Unpin 挤成一团 |
| Menu 键 | `flex` | 钉住之后它还在，而窄栏正盖在它身上 |
| 菜单那一列 | `mt-10` | Unpin 挪到底下之后，窄栏顶上多出一块无缘无故的 2.5rem 空白 |
| 面板的上下左右 | `inset-y-0 left-0` | 改成竖直居中的 bar 时，`top:0; bottom:0` 会把它继续拉满 |
| 面板的层级 | `z-50` | `.menu-pinned` 里那句 `z-index: 30` **从来没生效过**（实测 computed 一直是 50）—— 一句写了一天的死代码，而我还照着它写过一段解释「为什么盖得住」的注释。已删 |

规矩因此写死：**显隐或尺寸要随状态变的元素，那几个属性一律写在 app.css 上，
模板的 class 串里一个都不留。** 守卫
`SiteMenuTests.test_nothing_whose_display_changes_carries_a_display_utility`
只查这三个 —— 别的元素没有第二档，不会有人来盖。
⚠️ 那条守卫第一版咬到了**解释这件事的注释自己**（里面正引着 `flex items-center
gap-2`），加 `_blank_out_comments()` 之后才对。

### ⚠️ 菜单挂在两个顶层模板上，而我只改了一个

`base.html` 和 `home.html` 各有各的 `x-data`（首页多 `past` / `hover` 两个给顶栏
淡入用的）。只给 base 加 `pinned` 的表现是：**首页那颗钉住键整个报错**
（Alpine 的 "pinned is not defined"），而屏幕上看起来只是它没有字。

⚠️ 抓到它的是一条**看起来无关**的老测试（`test_the_theme_boot_still_reaches_the_
pages_that_want_it`）—— 它断言未登录首页上没有 `localStorage`，而新的引导脚本
把它带了过去。那条断言因此收窄成主题自己的 key：它问的仍然是同一件事，只是
`<head>` 里从此有两段引导脚本，而「有没有 localStorage」不再等于「有没有主题引导」。

两个外壳共用一份钉住的逻辑（`app.js` 的 `pinning()`），不是抄两遍。

### ⚠️ 状态写回 class 用 `$watch`，理由和讲次表那一处一模一样

`x-bind:class` 只移除它自己加过的 class，而这一个是引导脚本直接写进 `classList`
的；`bind` 又排在 `init` 之前。这两条本轮已经在讲次表那个开关上各踩过一次，
详见上面那一节 —— 这里一次都没踩，是因为那一节先写了。

#### 验收（浏览器，2026-09-14）

- [x] 1440px：窄栏在，七个图标各带小字，`Memories Photos` 断成两行不撑开
- [x] **Unpin 沉在窄栏最下面**（用户定的位置），条目少的时候也贴着底边而不是飘着
      —— ⚠️ 代价是键盘 Tab 先到它、再到那些链接，理由写在 app.css 那条规则上
- [x] **竖直居中的浮起 bar**（2026-09-15）：1440×900 上上下各 56px、高 565px
      （= 内容高，不再满屏）；417px 矮视口上封顶 393px、内部自己滚、不出屏
- [x] 最紧的那一档（报表页 `max-w-6xl`，1280px）：bar 右缘 80px、正文起点 80px，**压进 0px**
- [x] `.is-open` 和居中不打架（`translate` vs `transform`，实测 top 不变）
- [x] ~~Home 顶到最上面~~ 被同日那次「改成居中的 bar」取代；但它顺手修掉的那条
      过期理由仍然成立：原来那 76px 的让位是为了
      不盖住顶栏左上角那颗 Menu 键 —— 而钉住时那颗键自己就收掉了，所以**那条理由
      已经不成立**，让出来的只是一块空白。⚠️ 这是本轮第二条「理由随着页面上少掉的
      东西一起失效，而它读起来还很有道理」（第一条是讲次表那句 When）
- [x] 深色 + 大图那一档的窄栏：`rgba(0 0 0 / 0.62)` + 14px 模糊，底下换成很亮的
      颜色时白字白图标照旧清楚 —— 62% 的黑足够撑住对比度
      ⚠️ 走查时 `has-hero` 在库里是关的（没上传过首页大图），所以是在浏览器里临时
      加上那个 class 看的；真上传一张之后仍应复看一眼
- [x] Ministry Admin 那道分界线还在，文字转 `sr-only`（读屏的人照旧知道在哪一组）
- [x] 1440px：正文一个像素不动，也不被压住；顶栏那颗 Menu 键按设计收掉
- [x] 1024px：窄栏整个不生效，页面和加这个功能之前一模一样
- [x] 390px：面板照旧滑出，Pin 键不画，版心不溢出
- [x] 刷新之后还记得（`localStorage` + 首帧引导脚本，不闪）

## 三件事：地址与地图、筛选条重做、Events I Manage 进 tab 排（2026-09-15）

用户当轮提的三件，互不相关，一个分支做完。计划在 plan mode 里写过一遍，
逐步执行、每步开工前复查。

### 一、活动的街道地址 + 一条地图链接

🔴 **前提和最初设想不同，是查出来的**：`location` 存的**全是房间名**
（`Chapel` / `Room 2B` / `Back garden`），而全站**没有任何地方存着街道地址**
（`org.Ministry` 没有，`core.HomePage` 也没有）。所以这是**新增一层信息**，
不是给已有字段换个显示方式。做法和代价全部写进 design-system.md 六之二。

**开工前复查救回来的四条**（计划里一条都没写）：

| | |
|---|---|
| `Contact` 其实是**五个**地址字段 | 还有 `address_country`。这里**只加四个**，理由写进注释 |
| 搜索框的标签写着「name or **location**」 | 不把街道和城市加进搜索，那句话就成了每个字都对的假话 |
| 表单是逐个字段平铺渲染的 | 活动编辑页因此长四行，接受，写成已知代价 |
| `seed_demo` 里没有一条带地址 | 加了两条——一个看不见的功能没有人会去检查它 |

**执行中撞上的两条**（计划也没写，因为只有动手才看得见）：
`EventSeries` 也必须有这四个字段（`SHARED_PUBLISH_FIELDS` 同时喂两张表单，
加上去当场 `FieldError`），而更要紧的是**系列生成活动时逐字段复制** ——
地址不跟着走的话，一门每周的课十二个晚上在地图上全部打不开，
**而手工建的单场活动好好的**；以及 `postal_address` 要抽成只带行为的 mixin
（`Event` 和 `EventSeries` 共用，照 `ImmutableCodeMixin` 的先例）。

### 二、Events I Manage 进 tab 排

🔴 **只在管理页自己身上出现**（用户：管理页是它所管页面的**附属**），
Events / Programs 两页照旧两格；进去的入口仍然是列表页标题行右端那颗 ⋮。
所以 2026-09-03 那条「一个东西一个入口」**一点没被破坏** —— 这一排是到了
里面之后用来回到两边兄弟页的导航，不是第二道门。

⚠️ 第一版我做成了三页都显示、且撤掉了那颗 ⋮，被用户当场纠正。
⚠️ 那一格的名字**抽成了 `core.context_processors.manage_list_name()`**：
foundation tier 看到的那一页叫「All Events」，写死一个词会造出「这一排一个
名字、`<h1>` 另一个名字」（`ManageListHeadTests` 当场抓住了第一版）。

### 三、筛选卡 → 一条细 bar，并撤掉全部 collapse

版式照用户给的截图。**删掉 12 条测试**（9 条折叠 + 3 条对齐），而它们都不是
失效的断言，是**所守的机制被整个撤掉了**。理由和那次「2026-09-03 当天撤回」
的区别写在 design-system.md 那一行上。


## 计划外 · 同一个「红在日历上」的病，第三次（2026-09-14）

23:30 跑全量，`test_todays_meeting_keeps_its_own_state_underneath` 红了。
它摆的是「今天那一讲」，而摆法是 `NOW + HOUR` —— **23:00 之后的每一天，
一小时后已经是明天**，于是 `is_today` 为假。

⚠️ **窗口是每天一小时**，比前两次都窄：`ff06105` 那条是跨午夜的十几分钟，
报表那两条是「每个月有一天」。这一条一年里有 365 个小时会红，而它们都在
没有人跑测试的时段 —— 所以它大概率会以「某天早上莫名其妙红了一次」的形式出现。

🔴 **三次的共同形状写下来，因为它显然还会有第四次：
用「现在 ± 一段」去摆一个和日历边界有关的场景。**
写的人当天跑过、绿的，之后几十次也绿，直到某一次在错的钟点上跑。
正确的摆法是把时刻**钉死**（`day_start(local_today()) + N * HOUR`），
那也是这个仓库已有的惯例（`WhenLabelsTests` 顶上那段注释）。

⚠️ 顺带修的第二条是同一次改动逼出来的：`test_next_up_numbers_a_meeting_by_its_
place_in_the_course` 原来断言「meeting 3」，而第三讲一钉到上午十点，下午跑的
时候它已经过去、不在 Next up 里了。那一条测的是**序号怎么数**，不是哪一讲排在
最前 —— 所以它该用一个和钟点无关的那一次（第四讲）。
⚠️ 这是「钉死时刻」带来的第二笔账：钉死之后，**别的**依赖「它还没到」的断言
要跟着重新读一遍。

## 计划外 · 一轮假红，成因是我在测试跑着的时候改文件（2026-09-14）

全量跑出 10 条 ERROR，全在 `core.tests`，全是 `KeyError: 'list_noun'`。
而那一轮启动时，`list_noun` 这个东西**还不存在** —— 它是我在那一轮跑到一半时
加进去的。

🔴 **成因：Django 的模板是运行时从磁盘读的，而视图代码是进程启动时 import 的。**
   于是那一轮跑到 `core.tests` 那一段（时间上更晚）时，读到的是**新模板**
   （要 `list_noun`）配**旧视图**（不给它）—— 一个从未同时存在过的组合。

⚠️ 十条失败里没有一条是真的，而它们看起来完全像真的：报错点、堆栈、变量名
   全都指向一处真实的代码。分辨它的唯一办法是**重跑一次**，而我差点直接去修。

⚠️ 规矩因此写下来：**测试跑着的时候不改被它读的文件**。这个仓库的测试跑一轮
   将近五分钟，中间那五分钟很想顺手做点别的 —— 而顺手改的如果是模板、fixture
   或者任何运行时才读的东西，那一轮的结果就不可信了。
...
| [D45](decisions/D45-add-to-calendar.md) | 新开（2026-09-14，用户当轮追加）：加进个人日历，一门课整期 / 单讲两种下法，每条 VEVENT 自带一句「这份拷贝有多新」。⚠️ **「不走订阅」那半句同日被用户推翻**，补了一条每人一条私密地址的订阅源，见那份决定第二节后面补的一节 |
...
## L5.8b · simplify 留下的五条结构问题，做掉（2026-09-15）

`/simplify` 的四个 agent 共报 22 条，当场修了 9 条。剩下 6 条被判为「结构重构，
超出清理范围」而跳过；用户逐条看过白话解释之后指定做掉第 2–6 条（第 1 条维持
跳过）。全量 1831 → 1843 条，全绿。

**这五条的共同点：今天都不出错。** 它们是「下次有人改那一块时会悄悄漏掉」——
症状出现时离改动已经很远，而且多半不报错。

| # | 是什么 | 钉住它的守卫 |
|---|---|---|
| 2 🔴 | **订阅源抄了一份「该收到哪几讲」的规矩。** 它写在 `my_meetings()` 里（决定 17 自己挑晚上、决定 18 中途加入的人前几讲根本不存在），而订阅源为了把每门课两次查询降成常数把判据抄了一份，注释还写着「和它逐字一致」。改规矩的人会去改 `my_meetings()`（所有注释和测试都指向它），抄件不跟着变 —— **表现是某人手机日历里多出他没报的晚上**。他看得见，我们看不见，而这是唯一不认登录态的路由 | `FeedFollowsTheSameRuleAsTheDownloadTests` |
| 3 | **结果片段要两样配料，只包装了一样。** 数据由 `_listing()` 给，名词（`list_noun`）只由 `_list_page()` 给，于是每个渲染或 OOB 发这块片段的视图都要记得调两个。漏掉第二个渲染成 `No s for you in this period.`，而且**只在列表为空时看得见** | `ListingCarriesItsOwnNounTests` |
| 4 | **两套做同一件事的模具。** `_sibling_tabs()` 和 `_signups()` 各拼一份 `page_bar.html` 要的字典。加第四个键时只会加到一套上，而模板把取不到的键当空字符串 —— 两排标签长得不一样，而每一排单独看都正常 | `PageBarCellsComeFromOneMouldTests` |
| 5 | **同一个弹窗的外观写了三遍，而且漂移已经发生。** `.filter-menu` 头上写着「和日期弹层同一套外观」，而它的内边距和圆角实际跟着 `.row-menu`、只有阴影跟着日期弹层。**注释先变成了假话**，没有任何东西报警 | `PopoverShellIsOneRuleTests` |
| 6 | **看得见的菜单和把关的名单是两份。** Role 那一格的显示清单（`nature_options`）和字段的 `choices` 各从 `Nature.choices` 拼一遍，连「Any role」都各硬写了一次。任何一次「让选项跟着人或页面变」的需求都会落在把关那份上 —— 而显示那份不跟着变 | `RoleOptionsComeFromTheFieldTests` |

### 第 6 条那个症状值得单写：它**完全不报错**

人选了一个菜单里明明画着的选项 → 校验判它无效 → `is_valid()` 为假 →
筛选什么都没筛 → 页面列出**全部**活动，而按钮上还写着他选的那个 Role、
底下那行「Filtered by …」反而消失了。他的结论会是「这个筛选坏了」，
而不会想到是自己选了个不被接受的值。

### 第 2 条：计划里写错了一个接口细节，实施时才看见

计划打算让批量版返回「全部讲次」。而 `my_meetings()` 的 `None` 有**两种**含义 ——
「不是一门课」和「是一门课但他一行点名都没有」—— 两者都靠
`calendar_occasions(meetings=None)` 回落到活动自己的默认。改成返回列表的话，
`_calendar_response()` 里那句 `len(meetings) == 1` 会让**单讲课程的文件名多出一截
日期**。所以 `None` 的约定原样保住，订阅源侧写成
`picked[event_id] or all_sessions.get(event_id, [])`。
⚠️ 查询数实测未变：订阅源 4 次（和课数无关）、`my_meetings()` 2 次。

⚠️ 顺带一个**在坏数据上更严**的改动，如实记着：旧写法按「报名所属的活动」找讲次，
新写法按「讲次所属的活动」。`SessionAttendance.clean()` 规则 1 要求两者一致，
而裸 `create()` 走得过去 —— 好数据上两者逐字相同，坏数据上 A 课的讲次不会再
混进 B 课的文件里。

### 守卫这一侧，两条本轮学到的

⚠️ **神谕比相等更值钱。** 第 2 条最自然的守卫是「`my_meetings()` 和
`my_meetings_for()` 答案相同」—— 而在今天的写法下那是**厢语**（一个就是另一个的
包装），并且有人重新把规矩抄进订阅源时它照样是绿的。改成拿 `my_meetings()` 当
神谕去比**订阅源真正发出去的内容**，四种形状（挑过晚上的、中途加入的、
没有点名行的路人、单场活动）一次测完。反向验证时它正好点出那三讲。

⚠️ **子串不是类名。** `PopoverShellIsOneRuleTests` 第一版用 `".filter-menu" in
selectors`，当场被 `.filter-menu-row:hover` 匹上 —— 那是菜单里的**一项**，
不是弹层自己。改成 `re.escape(skin) + r"(?![\w-])"`。

⚠️ 五条各自都在旧代码上跑过一遍确认会红，其中第 4 条用的是「下一个人最自然会
写的那种写法」：给其中一排的格子加一个 `badge` 键。守卫报出「页头条的格子有两种
形状了」并指名哪两组页面。
