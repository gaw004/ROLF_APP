...
> 本文件是 `../goal.md` 拆出来的一条决策记录（2026-07-30 拆分）。
> **`goal.md` 仍是唯一入口**：决策一览表和「去哪找」都在那里，
> 代码注释里写的 `goal.md D15` 指的就是本文件。

> 2026-07-27 讨论的产物。起因是「紧急联系人为什么不直接在 Contact 上加两个字段？」
>
> **紧急联系人这一条 2026-07-28 当天改过两次，全部保留** ——
...
**不做"什么关系都能装"的通用表。** 判据是：
一种关系有自己的字段、自己的校验规则、或者对方不是主体 → 建专用表。
**都没有 → 它八成根本不是一张表，而是某张表上的一个字段**（例：一次活动的家长同意
不是 `Guardianship`，是 `Participation` 上的六个同意字段）。

通用表的卖点是"加新类型零成本 + 一次查询列出此人所有关系"，
而基金会需要的一直是具体的那几个问题 ——
「出事打谁的电话」（`EmergencyContact`）、「谁在哪个编制上」（`Assignment`）、
「这次活动谁同意了」（`Participation` 的同意字段）。三个问题三种形状，
一张通用表一个都放不下：专有字段和专属校验（同意书日期、能否代签、法定/委托）
它统统没处写，真需求一来还是要拆成专用表。

**判据一句话**：一张"以后什么都能装"的表，代价是现在就要付的复杂度，
收益是一个还没出现的需求。**先建看得见的那张表。**

关系标签（"母亲"/"邻居"/"同事"）复用 `RelationshipType` 词表，不新建 ——
一张词表两个使用方（`EmergencyContact.relationship_type`、
`Participation.consent_relationship`），才不会攒出"母亲/妈/mother"三种写法。
`RelationshipType` 的正反双标签让"以什么身份"这件事在两个方向上都读得通；
`usable_as_emergency_contact` 用 `limit_choices_to` 把无关类型挡在紧急联系人下拉之外，
同 `preferred_language` 已有的写法。
...
| 任职 | 专用表 `Assignment` | 有自己的字段（`status` / `employment_type`）和起止日期 |
| 紧急联系人 | 专用表 `EmergencyContact`，姓名/电话存文本 | **第四条判据** —— 形状上自引用 FK 完全放得下，但紧急联系人不该进 `Contact`。见下面「为什么最终选了文本方案」 |
| 一次活动的家长同意 | **不是表** —— `Participation` 上的六个同意字段 | 那是一条事件记录，不是一段长期关系。见 Phase B 的模型表 |
| 法定监护人 | 专用表 `Guardianship`，**但继续推迟** | 有自己的字段。⚠️ 原文写的是"基金会有没有同意书流程尚未答复"——[已经答复了：有](#-已答复2026-07-29有同意流程但仍然不建-guardianship)，**而答案并不指向建这张表** |
...
> 所以本条决策的历史是：纯文本字段 → 全 FK + `is_reference_only`
