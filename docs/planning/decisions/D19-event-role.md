...
## 2026-09-08 增补：一个人两个角色，在讲次那一层怎么读

本条允许一个人在一场活动里持有多行 `Participation`（他既搬东西又管接待，
[D28 第五节](D28-qr-checkin.md)为此让他在扫码时自己选）。
[L5.2](../06-roadmap.md) 之后这句话延伸到了每一讲：同一个晚上，他有两行点名。

**「以前的学员回来当助教、同时自己还在上这门课」是这条允许最该服务的那种人** ——
而它也正是第一版 [D43](D43-hours-given-and-hours-received.md) 算错的那一格：
「他接受到多少小时的服务」按报名行问，问两次得到两倍，
两小时的课算成四小时，而 12 小时门槛正是按这个数读的。

改法不是给那个数去重，是让它**只在 attending 的行上成立**：
助教那一行是来提供的，没有东西被交付给他。于是各角色各算各的：

| 他的这一行 | 给出去的工时 | 接受到的时数 |
|---|---|---|
| 助教（helping） | 由老师 / admin 填 | 不适用 |
| 座位（attending） | 那一列必须为空 | 这一讲的时长 |

⚠️ 所以本条那句「一个人可以有几行」在讲次这一层**不需要任何新规则**：
点名册的唯一约束是 `(报名行, 讲次)`，而两行报名是两个不同的事实，
本来就该各有一行点名。
