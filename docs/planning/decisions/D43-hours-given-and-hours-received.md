...
### 🔴 2026-09-08 更正：它**只在 attending 那一半**成立

初版写成角色中立的，理由是「助教也在那个教室里待了两小时」。那句话答的是
**「他在房间里待了多久」**，不是「他被服务了多久」—— 方向混了，而本条决定
整条就是关于方向的。助教是来提供的，没有任何东西被交付给他。

代价是具体的：一个人既当助教又占一个座位（[D19](D19-event-role.md) 允许，
而且「以前的学员回来帮忙」正是最该被留住的那种人），同一讲他有两行点名，
于是两小时的课算成四小时 —— 而 12 小时门槛正是按这个数读的。

⚠️ 这一条顺带取消了原计划的「按人去重」那个函数：只数 attending 之后，
一人身兼两职天然只有一行算数，那个重复不存在了，也就没有什么要去重。
helping 的行答 `None`（不是 0）：那一行上这个问题不成立。
...
| 推迟 | 什么时候再看 |
|---|---|
| 早退 / 迟到按实际时段折算 | 讲次签到已经在写 `checked_in_at` / `checked_out_at` 了（2026-09-08），所以条件已经到了一半 —— 差的是「按实际时段算」这个口径本身，等基金会说它需要 |
