...
## 六之三、撤销一条「还没开始」的授权：夹到它自己的起始日期（2026-09-18）

**结论：`end_date = max(今天, start_date)`**（用户拍板）。

⚠️ 这个 500 **早于本决策** —— `revoke_ministry_role()` 写「今天」和
`end_date >= start_date` 那条 CheckConstraint 在 `22d51a8` 上就是这样。
但它是在本轮做完之后才被查出来的，而且比一开始判断的严重：

🔴 **它不需要伪造输入。** 表单上「Starting on」那一格本来就收将来的日子
（「她下周一接手」是正常需求），而那一行发出来之后，旁边**照常渲染撤销键**
（模板只问 `end_date` 空不空）。点它 → 服务层写「到今天为止」→
终点早于起点 → CheckConstraint 拒绝 → 而这条路不走 `full_clean()`，
于是没有任何一条能落到表单上的路，`IntegrityError` 直接成为一个 500。
整条路径跑 HTTP 复现过。

**根因在动词上**：「撤销」承担了两件事 —— **终止**一件正在进行的事，
和**作废**一件还没开始的事。原来的代码只建模了前者。

夹住之后那一行是 `[9/25, 9/25)` —— 一个**空区间**，什么都不覆盖、永不生效，
而两个日期都如实留着：「从 9/25 起，9/25 撤销」读作「还没开始就被取消了」。
不删行，和这张表其余的规矩一致。

⚠️ 另外两个答案和它们各自的代价（都被否了）：

- **拒绝并给一句话** —— 现在**没有修改起始日期的入口**、也不许删行，于是那个人
  被卡死：一条他不想要的授权下周会自动生效，而他做不了任何事。除非同时加编辑
  入口，范围明显变大。
- **允许删行** —— 论据其实不弱（「永不删行」是为了保住「谁在什么时候能看这个」，
  而一条从未生效的授权对那个问题的答案是「从来没有人」）。否掉是因为它破的是
  一条一直很硬的规矩，而下一个人会拿它当先例。

守卫：两张表各一条 `test_revoking_a_grant_that_has_not_started_yet_cancels_it`，
都验红过（还原夹住那一行 → 两条当场 `IntegrityError`）。
