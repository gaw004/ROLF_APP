...
| [**D45**](D45-add-to-calendar.md) | 加进个人日历：下载一份，不做订阅 | 活动页一颗 Add to calendar，三个去处**不等价**且页面要说出差别：下载的 `.ics` 带稳定 UID（再下一次是**更新**已有那条），两条深链一次只加一个事件、**再点一次就多一条**；一门课两种下法都有（整期 / 单讲）；本质是往别人手机里放一份会过期的副本，所以每条 VEVENT 末尾都自带一句「这份不会自己更新，以活动页为准」；🔴 课的 SEQUENCE 要取 `max(Session, Event)` 两个时间戳 —— 只看讲次的话，改课名之后每一个日历永远显示旧名字，**重新下载也没用**；🔴 文件是私人的（挑过讲次的人只拿到他挑的那几讲），所以 `Cache-Control: private`；不放 ATTENDEE（转发即泄露身份，且有客户端会把它当成会议邀请）；否掉订阅式 feed 的是代价不对称 —— 一条令牌泄露等于那个人的全部行程 |
