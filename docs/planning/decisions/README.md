...
| [**D41**](D41-notices-are-not-events.md) | 公告是一张自己的表，不是活动上的一个开关 | 「停车场不能停车」「不再收支票」没有起止时刻，而 `Event` 两列都是 NOT NULL —— 编造时间之后它会进 R1 的活动条数、进日历、进 `.ics`；而**有**固定时间的那一半，决定 3 早就用一个 `attending` 角色答完了，于是 `takes_signups` 两头都不成立；⭐ `AUDIENCE_DAY = None` 意为**判在今天**，因为一条公告没有自己的那一天 —— 指向 `starts_showing` 会通过其余每一条测试，然后静默地对每一个新入职的人藏起公告；下架时间必填而下架 ≠ 丢失（`past()` 是去处）；🔴 没有 `EXPIRED`（时钟答得出来的事不进 status，`Event.status` 付过这笔学费）也没有 `WITHDRAWN`（提前下架分两种，分法是「有没有人看得到过它」）；受众轴为此搬进 `org/`，而含容那条规则留在 `events` —— 只有算术是通用的，句子不是 |

| [**D42**](D42-signed-in-landing-page.md) | `/me/` 是登录后的落脚点，落在一个**没有模型**的 app | 每张卡必须过四条才准建（说得出谁读它 / 天然不超四行 / 唯一一条通向已有页面的出口 / 明确的空态）—— 卡片这种形式天然鼓励再加一张，这四条是仪表盘不烂掉的唯一保险；🔴 **不放 `core`**，因为它要读 events / notices / org 三个 app，而 [D17](D17-app-layout.md) 写死「谁也不许反向 import」—— 聚合器必须在依赖链末端，而没有 `models.py` 的 app 长不出表，于是「仪表盘不拥有数据」是结构事实；**不推翻 [D25](D25-public-front-page.md)**（`/me/` 要登录，D25 的判据它是过的），且 `/` 登录后**不跳转** —— 那个跳转正是 D25 推翻掉的东西；Volunteer hours 以账本命名且只认一档（D36 那个和禁止存在），⚠️ 而原计划要复用的那个「报表已有的服务函数」**不存在**；侧栏月历是**跳转控件**不是第三个日历页面，且它的点子不能从被截断的卡片列表里取 |

| [**D43**](D43-hours-given-and-hours-received.md) | 给出去的时间和接受到的时间，是两个方向相反的数 | 「来接受服务的人也想知道自己被服务了多久」是对的，而答案**不是放开 `hours` 那一列** —— 那一列装的是人给基金会的时间，接受到的时数是反方向的，两者相加得到「我们收到的 ＋ 我们发出的」，一个没有定义、而且看起来完全合理的量（[D36](D36-two-hour-ledgers.md) 那条不变量的第四次应用）；🔴 **不加列，用算的** —— `Session` 已经存了每一次的起止两列，第三个数说「它们相差多少」就是一份可以和它们打架的真相，同 `Event.duration` / `scheduled_hours()`；⚠️ **不是教育项目专有的**（财务辅导、支持小组、职业培训一样成立），行业里也不叫课时 —— Salesforce PMM 那一列叫 `Quantity` 且单位可配，正因为它不假设你在办课；L1 那条性质轴**不推翻**，改的是它的措辞，[D38](D38-served-as-volunteer-or-work.md) 的 `not_applicable` 一个字不动（它问身份，不问时长）；⚠️ 没有讲次的活动上这个数**不是 0 是不适用**，而 `Sum` 的空值离被渲染成 `0` 只差一个 `or 0` |
