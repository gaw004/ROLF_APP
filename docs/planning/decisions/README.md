...
| [**D44**](D44-the-front-page-scrolls-into-the-dashboard.md) | 首页往下滚就是仪表盘，视图搬进 `dashboard` | 登录的人在 `/` 往下滚就进 Home，未登录的人看到的 `/` 一个像素没变；视图从 `core` 搬走，因为 [D17](D17-app-layout.md) 不许依赖链的根 import 自己的下游 —— 而「`dashboard` 出一个模板标签」那条路只是把同一份依赖**藏起来**：grep 查不到，且 `{% load %}` 失败时连匿名访客一起白屏，所以判据是**在启动时吵闹地失败，好过在最公开的那一页安静地 500**；hero 和仪表盘之间的**入场动画做过五版，2026-09-12 整套撤销**（逐帧刚体 → 一次性缓动 → 三维翻转 → 滚动驱动 → 模拟器调参之后的那一版），撤的是这件事本身而不是某一版没做好：「一模一样」的意思是两半都一个像素不动，而入场动画的全部内容就是让下半页在读者滚到之前处在**别的位置**上 —— 五次判定里有四次否掉的是效果而不是数值，所以这是一个「靠眼睛判定、判了五次都是否」的效果；守卫因此从「入场的 JS 不许逐帧跑」换成「入场不许回来」；接缝的水洗色取自照片底部一条的**色相**而**亮度被钉死**——实拍那张图底部是深藏青，照原样铺下去内容区顶上会压垮深色正文，同 [D26](D26-palette-from-the-hero.md) 钉亮度的那一招；工时改印两个数，而「本年度」的两半必须**各按自己的日期**算 —— 一刀切会把一门跨年的课整期算进它开课的那一年 |
| [**D45**](D45-add-to-calendar.md) | 加进个人日历：下载一份，外加一条订阅源 | 活动页一颗 Add to calendar，三个去处**不等价**且页面要说出差别：下载的 `.ics` 带稳定 UID（再下一次是**更新**已有那条），两条深链一次只加一个事件、**再点一次就多一条**；一门课两种下法都有（整期 / 单讲）；本质是往别人手机里放一份会过期的副本，所以每条 VEVENT 末尾都自带一句「这份不会自己更新，以活动页为准」；🔴 课的 SEQUENCE 要取 `max(Session, Event)` 两个时间戳 —— 只看讲次的话，改课名之后每一个日历永远显示旧名字，**重新下载也没用**；🔴 文件是私人的（挑过讲次的人只拿到他挑的那几讲），所以 `Cache-Control: private`；不放 ATTENDEE（转发即泄露身份，且有客户端会把它当成会议邀请）。⚠️ **同日推翻了「不做订阅」那半句**（用户当天要的）：每人一条私密地址 `calendar/<token>.ics`，**不认登录态**（抓它的是 Google / Apple 的服务器）。原来否掉它的三项成本一项都没消失，各给一个出口 —— 令牌可重新生成、界面上明写它是一把钥匙、内容每次请求都重新过一遍 `mine()`；而那句「这份不会自己更新」在订阅源里是假的，所以**换成**成立的那一句（`updates=` 开关，默认仍是下载那一档） |

| [**D46**](D46-position-code-is-an-optional-anchor.md) | `Position.code` 是可选的锚点，不是从名字派生的标签 | **一个格子做不了两件事：标识符不许变，描述不许过期** —— 一个从名字生成又冻住的 `code` 看起来像描述，却必然在某次改名之后变成一句假话（「身份证号不叫『北京-张三』」）；行业里两种模式，判据是**这个东西有没有一个人会分享出去的公开网址**（有 → 内容平台的 slug；没有 → 企业系统的可选人工键，Stripe `lookup_key` / SuccessFactors `externalCode`），而岗位页只有管理员进得去；带 ministry 前缀的自动生成**治好了「没含义」和「不可复现」两个毛病、却正好是那个「北京-张三」**；当初给 `Position` 加 `code` 的理由（`name` 故意不唯一，代码要有个锚点）是一个**预期**，而到今天为止唯一的读者是 `seed_demo` 那五句 —— 而那五个手挑的值正是本条要保护的用法；⚠️ 归一化必须把 `""` 收成 `None`，否则 `UniqueConstraint(Lower("code"))` 只放行一行空的 |

| [**D47**](D47-event-level-grant.md) | 单场活动的管理授权：第三张表，不是 `MinistryRole` 上的一档 | [D20](D20-ministry-role.md) 自己的判据（「句子里含不含『某个 ministry 的』」）**过不了**「某**一场活动**的完全管理权」—— 它比一个 ministry 窄，所以是第三档、第三张表；而「完全一致」几乎是免费的，因为 `can_manage_event()` 是全项目唯一的写判断 —— **散在各个 view 里的话这是二十处修改，而漏掉的那一处是静默的**，这是 D20 第一次真的被兑现；⚠️ 但有一处**配对**（管理列表上那个集合版判断）差点被漏掉；发布窄、收回宽（照公告那一对：一条不该再有的权限等不了授权的 admin 接电话）；🔴 落地时撞上一个**既有**问题 —— 撤销把 `end_date` 填成今天，而右闭语义让它**明天**才失效，`MinistryRole` 一直如此且没有任何测试钉着；敢改成右开，是因为授权表单上**没有截止日期那一格**（用户拍板），于是那一列只有撤销一个来源 —— 而第一版试过用 `updated_at` 当判据，**不成立**（`auto_now`，任何保存都会写它），被测试当场抓到；授权**管到活动标成 Wrapped up 为止**，于是不会攒下一批永远看得见未成年人紧急联系电话的人 |
| [**D48**](D48-foundation-tier-writes.md) | 基金会那一层能写，而不只是能读 —— 推翻 2026-08-05 | 触发它的不是「想要更多权限」而是一个**陷阱**：只放开发布的话，foundation admin 发得出活动、**下一秒 403**，开不了工种、发不了通知，发出来的是一个谁也报不了名的壳 —— 所以真正的问题是「发布」这个动词包含什么，答案是包含把它办起来；⚠️ 代价**先量过再拍板**（实测打红 11 条，其中 7 条属于一个专门为这件事写的类），因为「拆掉一个带测试的既有设计」和「加一个功能」是两件事；🔴 **一条断言都没删** —— 整个类改名反过来钉（「不能签到」→「能签到」），而**手法一处没反**：仍然走 POST 不读 HTML，撤的是「读得了改不了」那一档、不是「画不画按钮不是权限」那条道理；`can_view_event_records()` 和 `can_manage_event()` 从此答案相同但**不许合并** —— 它们分开过、还是往两个方向分的，合并会把巧合固化成规律；🔴 一句只有**浏览器走查**才抓得到的假话（横幅写着「publishing… stay with each ministry's own admins」，而我 grep 的是「只读」——**一句假话不会用关键词标注自己**）|
| [**D49**](D49-program-meetings.md) | 一门课的讲次，由一条规则在发布时排完 | 它补的是一个**半个功能**：`Session`、`Session.clean()`、`add_session()` 从 L5 起都在，缺的是**门** —— 全站没有一条 URL 到得了它，发布一门课得到的是一个有起止日期、一讲都没有的壳；和 `EventSeries` 共用选择器但**共用的是「怎么问」不是「存不存」**（课不存规则，因为它没有第二个读者）；学期的两端是**推**出来的 —— 问一遍就是同一件事有两个来源，症状是「我填了 12 次、只排出来 9 次」而不报错；「不结束」那一档不存在的理由是**硬的**（`end_time` 是 `NOT NULL`）；展开**只有一次**，否则「页面说 12 讲、按下去排了 13 讲」；删一讲的判据用现成的 `register_kept_at()` —— **两扇门一条判据**；🔴 执行时踩到四件事，其中「纯 mixin 上声明的表单字段会被整个丢掉」之所以**吵**纯属运气 |
| [**D50**](D50-address-input.md) | 地址在录入的那一刻就校验，而且不靠 JavaScript | 这四格的**唯一去处**是「在地图里打开这个地方」，而一个地图打不开的地址**不报错、只是打不开** —— 发现它的是那天开车去的人；州是一个**真的** `<select>` 而不是 `contact` 那边那段 JS 换出来的（D24：功能不挂在脚本上），而活动这一侧**不需要**那条路，因为 `Event` 没有 `address_country` 列，州不是一条有条件的规则 —— 「不加国家」和「校验成美国州」是同一个前提的两面；⚠️ `contact` 那一侧本轮**没动**，而这件事写在 `core/address.py` 的模块注释里而不是留给人发现（**一条只说一半的注释比没有更糟**）；🔴 最值钱的是**去空白** —— `"NY "` 和 `"NY"` 是两个值而屏幕上一模一样；两格**不许变成必填**（「地址可以晚一点补」），有测试钉着；换在 `PublishFormMixin` 上一处改三处生效 —— 漏掉一张的症状 9-15 刚踩过（一门课十二个晚上在地图上全打不开，而单场活动好好的）; ⚠️ 地图自动补全**拍板不做**，理由和重启条件在 `deferred.md` |
| [**D51**](D51-date-ranges-are-half-open.md) | 日期区间右开：`end_date` 是第一个不算数的日子 | 同一条日期规则此前有**四种写法**（`active()` / `in_force()` / `is_currently_active` / `is_in_force`），而其中两种是分两天加的 —— 中间那一天撤销当天那一行写着「In effect: Yes」，正下方横幅写着「即刻生效」，两边都不报错；用户拍板把**语义轴整个消掉**（事实「做到 15 号」存 16 号，授权「今天撤销」存今天），于是只剩「问一批行 / 问一行」两种求值、零个开关；右闭那一侧的理由（「工时会少算一天」）**查无实据** ——全项目没有一处拿 `end_date` 做算术；顺带把授权表那条键在 `start_date` 上的唯一约束换成区间排他约束，「恢复」分支（清掉 `end_date`、造出一段从未存在过的连续授权）随之删掉<br>↳ [唯一的不变量](D51-date-ranges-are-half-open.md#-唯一的不变量一条规则两种求值零个开关) · [为什么不是参数化](D51-date-ranges-are-half-open.md#一为什么不是把语义轴参数化) · [显示的两个口径](D51-date-ranges-are-half-open.md#四显示存一条规则展示两个具名口径) · [没有做的事](D51-date-ranges-are-half-open.md#七没有做的事) |
