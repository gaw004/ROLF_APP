...
| ~~请假 / 停职的历史（`Leave` 表）~~ | ✅ **2026-08-14 出栏，进 Phase D** —— 重启条件真的到了，而且是被班表逼到的：有了稠密的 `Shift` 之后，「两周假手工去翻 10 行班次」没有人会真的执行。形状和这一行当初写的一字不差。见 [D32 第五节](decisions/D32-worker-axes-schedule-and-assignment.md) | — |
| ~~`Position.headcount`（编制人数）~~ | ✅ **2026-08-14 出栏，进 Phase D** —— 基金会问「每个职位多少人」时包含了"**应该**有几人"。照这一行当初写的做：加一个整数字段，不改结构，`vacant()` 改成比较在职人数与 `headcount` | — |
