...
| 订阅式日历 feed（webcal） | 它能自动更新，而下载那一份不能（[D45](decisions/D45-add-to-calendar.md)）。否掉它的是代价不对称：日历客户端不带登录态，所以要给每个人一条**长期公开**的带令牌 URL，而一条令牌泄露 = 那个人的全部行程 —— 泄露的方式包括「把日历分享给家人」。⚠️ 换来的只是「改期之后不用重下一次」，而 D22 那套改期通知恰好就是「重新加一次」的入口 | 有人反映改期之后日历里还是旧的 |
