...
| ~~`Event.parent`（活动系列）~~ / 奖励规则 | 奖励规则要等基金会说清楚，且是"加字段不改结构"。**关键是它所需的维度已经全部存下来了**（哪场 `Event`、哪个 `EventRole`、多少 `hours`）。⚠️ `Event.parent` **2026-09-08 出栏**，见下面那一行 | 见下面各自那一行 |
...
| ~~活动的分组 / 系列（`Event.parent`）~~ | ✅ **2026-09-08 出栏，进批三 L5** —— 而**当初那个载体判定作废了**，这才是这一行值得留着读的部分。原文写的是「按 D15 三条件检验：最多一个父、无独立属性、只有一种类型 → 自引用 FK 正是对的载体」，其中第二条**当场就破了**：一个系列携带**生成规则**（RRULE、起始日、时长、还有一整份角色模板），而那正是「关系自己有属性」。D15 自己盯着这一格并明写「条件破了就必须升级成表」。<br>⚠️ 检验没错，**是被检验的东西变了** —— 写这一行时想的是「上午场 / 下午场」这种纯分组，而基金会真正要的是「按规则生成十二场」。同一个词（"系列"）指向两个形状不同的东西，和 `Guardianship` 那一行是同一类误读。<br>落点：独立的 `EventSeries` 表（recurring events），以及 `Event` + N 个 `Session`（Programs）。见 [`06-roadmap.md`](06-roadmap.md) L5.4 / L5.1 | — |
...
| 邮件**群发 / 营销**（简报、募捐信、全员公告） | 和内部管理是两回事，且要处理退订、名单管理、合规。<br>**2026-08-31 收窄：站内公告板不在这条推迟里**（[D41](decisions/D41-notices-are-not-events.md)）。分界是**拉还是推** —— `/notices/` 是一块人自己去看的板子，退订、名单、合规一样都不付；这一行推迟的是**推**到收件箱里去的那件事。<br>**注意它和 P6 不是一回事**：P6 是**事务性**通知（这场活动改时间了，通知**这场活动的报名者**），范围由 `Participation` 天然界定，触发条件明确。群发是"给所有人发点什么"，没有边界 | 基金会真的要做简报时 |
| 活动列表顶部的公告横幅 | 一条压在 `/events/` 列表顶上的横幅在这一版视觉里不好看，而为一个**还没出现过**的问题先弄脏那一屏，代价是每天都在付的。⚠️ 于是活动侧通向公告的唯一线索是活动详情页角色空状态里那一句，它因此承重 | 真的发生过一次「公告发了，来的人却不知道」。⚠️ **那时不必默认做横幅** —— 顶栏一个带计数的入口、或者只在**有没读过的**公告时才出现的一条窄带都是候选，后者要多一张 `NoticeRead` 表（[D41 第七节](decisions/D41-notices-are-not-events.md)） |
...
| admin 移除某人的报名 + 通知本人 | **2026-09-08 谈过，判它现在不做。** 今天**没有任何人做得到，而且是故意的**：`participation_cancel` 的作用域锁死在本人（别人的行 404），报名管理页没有移除控件，基金会那一档只有 `events.view_participation`（只读），ministry admin 又被 `StaffOnlyAdminMiddleware` 挡在 `/admin/` 外面，而 Phase C 明写不给 delete 权限。<br>真正会逼出它的是**安全场景**（系统里存着未成年人的姓名、生日、地址、紧急联系人和同意记录）；其余场景（报重了、报错场次、名额）都能靠沟通解决 —— 而「请他自己退出」恰恰是安全场景唯一不能用的办法。<br>🔴 **重启前必须先回答的那个问题**：`services.sign_up()` 遇到同一岗位上一条 `CANCELLED` 的行会**直接复用它**，所以不配上「挡住再报」，移除等于没移除，功能是假的。而「挡住」挂在哪儿（这一场活动 / 这个 ministry / 这个人）是一个比按钮本身大得多的决定。<br>⚠️ 形状上它**不是删行**：`cancel()` 是改状态，理由写在它自己的注释里（「报了名又退出」和「从来没来过」是两件事）。而「机构请他退出」是第三件事，可能要第三档状态 —— 那会动四处按状态过滤的地方 | 基金会说出一个「必须挡住某人再报名」的真实场景时 |
| 一条守卫，挡住漏收窄的 `Contact.objects.filter()` | [`participants.md`](participants.md) 第九节点名要过它，而它一直没人写（2026-09-08 记进来，之前**没有任何出栏去处**）。受助者和志愿者共用 `Contact` 之后，一句忘了收窄的查询会把志愿者通讯发给受助者。<br>⚠️ 今天不漏的理由仍然成立、也查证过：`services.resolve_recipients()` 走 `Participation` 不走 `Contact`，所以收件人天然由「谁报了这场活动」界定。推迟的是**把这件事钉住**，不是修一个现存的漏。<br>形状：一条 grep 守卫，照 `AudienceIsAskedGuardTests` 那一套 —— 难点在信号选什么，`Contact.objects.filter(` 太宽（合并页、admin、搜索都在用），要么按「函数体里同时出现收件人词汇」缩，要么等第一个真正的群发调用方出现 | 出现第二个按 `Contact` 取收件人的调用方时，或者做邮件群发那一步时 |
...
| 投票 / survey（内部员工表决、征集意见） | **2026-09-15 问过，判它现在不做。**<br>⚠️ 基金会问的是「能不能改一改 Event，让员工不选 sign up 的工种、改成选给定的选项」，并且自己补了一句「又觉得这样有点让 event 做太多不是自己的事情」—— **那句怀疑是对的**，而 [D41](decisions/D41-notices-are-not-events.md) 已经为同一件事论证过一遍：`Event.start_time` / `end_time` 都是 NOT NULL，给一次投票编一个时间之后，它会进 R1 的活动条数、进 ministry 报表、进日历、进 `.ics`、进两张列表页，**每一处都是错的**；而 `EventRole` 带着 `needed_count` / `nature` / 受众含容规则，`Participation` 带着工时、出勤、扫码签到、家长同意、`served_as` —— 一个「选项」一样都用不上。<br>🔴 还有一条 D41 没遇到的：`services.sign_up()` 遇到同一位置上一条 `CANCELLED` 的行会**直接复用它**（它自己的注释写着），而「改票」会撞上这个为报名设计的行为。<br>**真要做时的形状**（已经想清楚，不用重新设计）：一张自己的小表，**复用受众轴**（`org/audience.py`）—— 「foundation 内部员工」这件事它已经会说了（`visible_to_all_staff` / `visible_to_ministries`），不用发明新的权限概念。`Poll`（受众 + ministry + 起止 + draft/published，⚠️ 截止时间**必填**，理由照抄 D41 第三节：不会自己清空的板子没人再看）/ `PollOption` / `PollResponse`（唯一约束决定单选还是多选）。它会是第四张带受众的表，而搬家的钱 2026-08-31 就付过了。<br>⚠️ **「投票」和「survey」是两个产品**：投票 = 一个问题、若干选项、结果是计数，三张小表；survey = 多题、题型、量表、自由文本、导出 —— 那是一个表单构建器，和「邮件群发」那一行同一个量级，真有需求时用一条 Notice 链到外部工具。<br>⚠️ 入口照 [`phase-d.md` 第四节](phase-d.md)那张表的规矩来：[D42](decisions/D42-signed-in-landing-page.md) 的 `/me/` 是登录后的落脚点，**投票没有入口就没人投** | 基金会真的要投一次的时候。⚠️ 那时先确认要的是**投票**还是**survey** —— 上面那一格的最后一条 |

| 三条**在午夜前后会自己变红**的既有测试 | 2026-09-15 撞到（D47 落地时跑全量）：`RoleTopUpTests.test_an_evening_that_has_happened_gets_no_new_job` 以及 `SeriesChangesTests` 那两条。⚠️ **和那一轮的改动无关，验过**：把改动 stash 掉之后照样红。<br>成因是 `events/tests.py` 顶上那条注释警告过的同一类东西：`NOW` 是模块导入时取的 `local_now()`（UTC 的瞬间），而夹具按 `local_today()`（当地的那一天）往前推若干天。**UTC 已经翻页而当地还没有**的那几个小时里，「跨越今天」那个前提就不成立了。<br>⚠️ 另外两条单跑是绿的、只在全量里红 —— 那是测试之间的相互影响，要单独查。<br>**为什么不顺手修**：这是三条测试的夹具问题，而它落在 recurring events 那一整块上；在一轮讲权限的改动里顺手动它，会把「这一轮改了什么」弄浑 | 下一次动 recurring events 时一起修。⚠️ 修法大概率是**让夹具用同一个基准** —— 要么都走 `local_now()`，要么都走 `local_today()`，而不是一半一半 |
...
| ~~订阅式日历 feed（webcal）~~ | ✅ **2026-09-14 出栏，见 [D45 第二节](decisions/D45-add-to-calendar.md)** —— ⚠️ **不是重启条件到了**（没有人反映改期之后日历还是旧的），是**用户当天直接要了它**。原来那段理由里的三项成本一项都没消失，处理方式是各给一个出口：令牌可**重新生成**（`CalendarFeed.rotate()`，配确认页）、界面上明写「这条地址等于一把钥匙」、并且内容每次请求都重新过一遍 `mine()`。⚠️ 那句「泄露的方式包括把日历分享给家人」**仍然成立**，只是现在收得回来 | — |
