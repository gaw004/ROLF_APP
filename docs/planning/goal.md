...
- **关系数据完整性的三个缺口（A7 的约束没覆盖到，本阶段补）**：
  ...
  | 2 | `RelationshipType.name_a_to_b` 没有唯一约束 | 能建两个一模一样的 "parent of"，admin 下拉出现两个同名选项，选哪个都对但数据分裂成两半 | 加真正的 `UniqueConstraint`（按 D14 配 `clean()` 提示） | **强制层** |
  | 3 | 对称类型 a/b 调位 | `(王强, 李梅, spouse)` 和 `(李梅, 王强, spouse)` 同类型只是调了个位，唯一约束同样不触发 | 存的时候归一化：对称类型一律把 id 小的放 `contact_a`，加测试钉住 | 只能到提示层 —— 见下面的诚实说明 |
  ...
  > 真要强制，得在 `Relationship` 上冗余一个"是否对称"的布尔列才能建条件索引 ——
