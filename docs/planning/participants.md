...
| 缺口 | 什么时候再看 |
|---|---|
| ~~🔴 `Event` 装不下 (b) 类：跨多个场次的长期报名关系~~ | ✅ **2026-08-26 出栏**，重启条件真的到了 —— 基金会原话「报名一次就代表后面都报过了」。⚠️ 而当初那句「一场 `Event` 只有一个时刻」**不准确**：它一直有起止两列，缺的是中间那些时刻。所以形状不是「一个系列 + N 场活动」，是**一个 `Event` + N 个 `Session`**。见 [`06-roadmap.md` 批三](06-roadmap.md) |
...
L2.7（批二的浏览器验收）以及 L5.3 之后的每一步仍然没做，形状在第六节。
