...
| ⚠️ 缺勤率只在有人真去标的活动上有意义（2026-08-05 定） | `absent` 现在有写入路径了，但**没人标的活动会显示 0% 缺勤，和真的没人缺勤长得一模一样**。和工时那条偏差是同一个形状：数字跟着「哪个 admin 认真」走，不是随机误差。所以第一版**没有把缺勤率放进报表** —— 它只出现在签到页上 | 实际用过一轮之后。要加的话必须同时印「基于 N 场有人标注过的活动」，否则这个数会被当成事实读 |
