...
| 注册不加验证码 / 邮箱验证（2026-08-03 定，2026-08-06 修订） | 现在有**注册限流**了（per-IP 和全站两个桶，见 `core/ratelimit.py`），所以脚本批量灌库挡住了。⚠️ **挡不住的是有人用不属于自己的地址手工注册** —— 那要邮箱验证，而邮箱验证依赖真实发信，按「依赖上线后才存在的东西现在不做」推迟了（见 [`revisions.md`](revisions.md#十2026-08-06-修订记录了什么前端自查输入上限--登录名--分组菜单)）。代价不变：这样的账号能看到全部已发布活动，且在 `Contact` 表里留行 | **公开放开注册之前必须补**邮箱验证和验证码。做法见 [`03-roadmap.md` 的 C3.9](03-roadmap.md#c39-登录限流) |
| ⚠️ 改 email 不重新验证（2026-08-06 定） | email 现在就是登录名，而「我的资料」允许本人改它，**改完不发确认信**。代价如实说：一个人可以把自己账号的登录地址改成任意地址 —— 包括别人的。这主要是自伤（他自己会登不进去），但也能用来占着别人的地址。严格做法是 `pending_email` 字段加第二套确认流程，它依赖真实发信，所以和注册邮箱验证同一批做 | 发信通了之后（[C3.3](03-roadmap.md#c33-真实发信)），和上面那条一起处理。⚠️ **两条要一起做** —— 只做注册验证不做改 email 验证，等于留一道现成的绕路 |
