...
| ⚠️ 改 email 不重新验证（2026-08-06 定） | email 现在就是登录名，而「我的资料」允许本人改它，**改完不发确认信**。代价如实说：一个人可以把自己账号的登录地址改成任意地址 —— 包括别人的。这主要是自伤（他自己会登不进去），但也能用来占着别人的地址。严格做法是 `pending_email` 字段加第二套确认流程，它依赖真实发信，所以和注册邮箱验证同一批做 | SES 通了之后，和上面那条一起处理。⚠️ **两条要一起做** —— 只做注册验证不做改 email 验证，等于留一道现成的绕路 |
