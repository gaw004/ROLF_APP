...
| ⚠️ 签到点错了行没有撤销的办法（2026-08-05 定） | 新加的「No-show」按钮**故意不兼任这个** —— 它对已签到、已记工时的行直接拒绝。理由：「撤销我点错的」和「记录他没来」是两件不同的事实，一个按钮同时干这两件会让第二件变得没法读（[D27](decisions/D27-ministry-report.md)）。⚠️ 这个缺口在此之前就存在，不是新增的：签到从来就没有撤销路径。眼下的退路是 admin 站改那一行 | 试点里真的点错过一次、而 admin 站又不在手边时。那时加的是一个明确的「Undo check-in」，不是把它塞进 No-show |
