...
#
#    实测出来的：24 格里有四个粉（#fbd8dd / #e9bdd8 / #fdb9cf / #f5b6bb）。
#    按下标算它们是四个不同的颜色，屏幕上是同一个 —— 两张挨着的卡各配一个，
#    读起来仍然像一张。第一版就是这么错的，而它「通过」了不撞色的测试。
#
# ⚠️ 这张表是**对色值的判断**，所以它和 app.css 里那 24 行是一对。改一个色值
#    就要重新看它还属不属于原来那一族。守卫只钉得住「每一格都归了族」，
#    钉不住「归对了族」—— 后者只有眼睛看得出来。
COLOUR_FAMILIES = (
    (0, 9, 19),        # 蓝
    (1, 16, 18, 22),   # 粉
